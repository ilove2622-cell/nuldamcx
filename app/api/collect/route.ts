...
export async function POST(req: Request) {
  try {
    ...
    if (domain.includes('localhost')) {
      ...
      console.log(`[전체 수집] localhost 감지 → 도메인 교체: ${domain}`);
    }
    ...
    console.log(`[전체 수집] ① 사방넷으로 보낼 xmlUrl: ${xmlUrl}`);
    console.log(`[전체 수집] ② 최종 요청 URL: ${sabangnetApiUrl}`);

    const response = await fetch(sabangnetApiUrl, { method: 'GET' });

    console.log(`[전체 수집] ③ 사방넷 응답 status: ${response.status} ${response.statusText}`);
    ...
    for (const item of dataList) {
      ...
      if (!num) {
        console.log(`[전체 수집] NUM 없는 항목 스킵`);
        continue;
      }
      ...
      if (existing) {
        console.log(`[전체 수집] 이미 존재 → 스킵: NUM=${num}`);
        continue;
      }
      ...
      if (error) {
        console.error(`[전체 수집] Supabase insert 실패 NUM=${num}:`, error.message);
      } else {
        console.log(`[전체 수집] ✅ insert 성공: NUM=${num}`);
        newCount++;
      }
    }
